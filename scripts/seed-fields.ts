...
const STANDARD_FIELDS: FieldSeed[] = [
  ...
  { fieldName: "elevator_width_cm", scope: "PROPERTY", valueType: "NUMBER", labels: { en: "Elevator width", nl: "Liftbreedte", de: "Aufzugsbreite", fr: "Largeur d'ascenseur" }, unit: "cm" },
  { fieldName: "corridor_min_width_cm", scope: "PROPERTY", valueType: "NUMBER", labels: { en: "Minimum corridor width", nl: "Minimale gangbreedte", de: "Minimale Korridorbreite", fr: "Largeur minimale de couloir" }, unit: "cm" },
  ...
  { fieldName: "automatic_door", scope: "PROPERTY", valueType: "BOOLEAN", labels: { en: "Automatic door", nl: "Automatische deur", de: "Automatiktür", fr: "Porte automatique" }, searchFilter: true },
  { fieldName: "path_to_entrance", scope: "PROPERTY", valueType: "ENUM", enumValues: ["step_free", "uneven", "steep"], labels: { en: "Path to entrance", nl: "Pad naar ingang", de: "Weg zum Eingang", fr: "Chemin vers l'entrée" } },
  ...
  { fieldName: "visual_alarms", scope: "PROPERTY", valueType: "BOOLEAN", labels: { en: "Visual alarms", nl: "Visuele alarmen", de: "Visuelle Alarme", fr: "Alarmes visuelles" }, searchFilter: true },
  ...
  { fieldName: "step_free_room", scope: "ROOM", valueType: "BOOLEAN", labels: { en: "Step-free room", nl: "Drempelloze kamer", de: "Stufenfreies Zimmer", fr: "Chambre sans marche" } },
  { fieldName: "clear_space_beside_bed", scope: "ROOM", valueType: "BOOLEAN", labels: { en: "Clear space beside bed", nl: "Vrije ruimte naast bed", de: "Freiraum neben dem Bett", fr: "Espace libre à côté du lit" } },
  ...
  { fieldName: "room_types_available", scope: "PROPERTY", valueType: "ENUM", enumValues: ["single", "double", "twin", "suite", "family"], labels: { en: "Room types available", nl: "Beschikbare kamertypes", de: "Verfügbare Zimmertypen", fr: "Types de chambres disponibles" } },
...
/** Retired from the active audit catalogue — keep rows, mark inactive. */
const INACTIVE_FIELDS: FieldSeed[] = [
  { fieldName: "elevator_floor_count", scope: "PROPERTY", valueType: "NUMBER", labels: { en: "Elevator floors", nl: "Aantal liftverdiepingen", de: "Aufzugsetagen", fr: "Étages desservis" }, active: false },
  { fieldName: "accessible_room_count", scope: "PROPERTY", valueType: "NUMBER", labels: { en: "Number of accessible guest rooms", nl: "Aantal toegankelijke gastenkamers", de: "Anzahl barrierefreier Gästezimmer", fr: "Nombre de chambres accessibles" }, searchFilter: true, active: false },
];

async function upsertField(field: FieldSeed) {
...
async function main() {
  ...
  for (const field of INACTIVE_FIELDS) {
    await upsertField(field);
  }
