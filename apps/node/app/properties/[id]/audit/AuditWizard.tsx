...
import { mergeKnownCustomRoomTypes, flattenAuditPhotos, countAuditPhotos, stepScopeKey } from "@wikitraveler/audit";
import { type ExistingFact } from "./ExistingDataPanel";
import { RoomAuditSection } from "./RoomAuditSection";
import { AuditPhotoGallery } from "../../../components/audit/AuditPhotoGallery";
import { ExistingStepPhotos } from "../../../components/audit/ExistingStepPhotos";
import { PhotoLightbox } from "../../../components/audit/PhotoLightbox";
import {
...
function countPhotos(
  ...
  return countAuditPhotos(propertyPhotos, roomPhotos);
}
...
function flattenPhotos(
  ...
  return flattenAuditPhotos(propertyPhotos, roomPhotos);
}
...
export function AuditWizard({
  ...
  async function submit() {
    ...
    const res = await fetch(`/api/properties/${encodeURIComponent(propertyId)}/accessibility`, {
      method: "POST",
  ...
  function renderPropertyPhotoSection(step: PhotoStepId) {
    ...
    return (
      <div className="fk-audit-photo-section">
        <ExistingStepPhotos photos={prior} />
        <p className="fk-audit-photo-section__title">{t("ui.auditStepPhotos")}</p>
  ...
  return (
    <>
      <div className="card fk-audit-form">
        <div className="fk-audit-form__header">
          <span className="fk-audit-form__step-label">
            {t("ui.auditStepOf", { current: stepIndex + 1, total: steps.length })} · {t(STEP_TITLE_KEY[currentStep])}
          </span>
          {draftSavedAt ? (
            <span className="fk-audit-form__draft">{t("ui.auditDraftSaved")}</span>
          ) : null}
        </div>

        <div className="fk-audit-form__progress" aria-hidden="true">
          <div
            className="fk-audit-form__progress-bar"
